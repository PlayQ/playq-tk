#!/bin/sh
coursier launch com.lihaoyi:ammonite_2.13.6:2.4.0 --fork -M ammonite.Main -- sbtgen.sc true $*
exit
!#
...
@mainargs.main
def entrypoint(disableAmmoniteNativeParser: Boolean, args: String*) = {
  Entrypoint.main(new TkBuild(_ => ".").root, settings, Seq("-o", ".") ++ args)
}
...
object Targets {
  ...
  private def getScalaVersion(v: String): String = {
    s"""scala_$v.*"(.*)"""".r
      .findFirstMatchIn(
        Files.readString(FileSystems.getDefault.getPath("project/TkVersions.scala"))
      ).map(_.group(1)).getOrElse(throw new RuntimeException(s"Couldn't get `scala_$v` version from project/TkVersions.scala"))
  }
...
object Groups {
  final val tk = Set(Group("tk"))
}

final class TkBuild(rootOf: String => String) {
  ...
  object lib {
    ...
    final val d4sTest = ArtifactId("d4s-test")
    final val d4sCirce = ArtifactId("d4s-circe")
    ...
    final val basePath = Seq(rootOf("playq-tk"))
  }
  ...
  final lazy val tk_agg = Aggregate(
    name = ArtifactId("tk"),
    artifacts = Seq(
      Artifact(
        name = lib.tkMetrics,
  ...
  final val root = Project(
    ...
    appendPlugins = Defaults.SbtGenPlugins ++ Seq(
      SbtPlugin("io.7mind.izumi", "sbt-izumi-deps", Version.VExpr("Tk.PV.sbt_izumi_deps")),
    ),
  )
}
