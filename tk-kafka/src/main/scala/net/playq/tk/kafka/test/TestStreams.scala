...
import izumi.fundamentals.platform.resources.IzResources._
...
trait TestStreams[F[_, _]] {
  def resourceKafkaStream[A: Decoder](file: String): F[Throwable, Stream[F[Throwable, ?], KafkaData[A]]]
  def resourceJsonStream[A: Decoder](file: String): F[Throwable, Stream[F[Throwable, ?], A]]
}
...
object TestStreams {
  final class Impl[F[_, _]](
    ...
    private[this] def kafkaStream[A: Decoder](inputStream: InputStream): Stream[F[Throwable, ?], KafkaData[A]] =
    ...
    private[this] def jsonStream[A: Decoder](inputStream: InputStream): Stream[F[Throwable, ?], A] = {
      val globalPool = Blocker.liftExecutionContext(scala.concurrent.ExecutionContext.Implicits.global)
      fs2.io
        .readInputStream[F[Throwable, ?]](inputStream.pure, 64 * 1024, globalPool)
        .pipe(fs2.text.utf8Decode)
        .pipe(fs2.text.lines)
        .filter(_.nonEmpty)
        .evalMap[F[Throwable, ?], A] {
          json =>
            parser
